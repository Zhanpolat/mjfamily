...
const Kabina = ({ color, handlColor, zanet }) => {

    const stol = [
        {
            svg: <svg xmlns="http://www.w3.org/2000/svg" width="238" height="114" viewBox="0 0 238 114" fill="none">
        ...
        {
            svg: <svg xmlns="http://www.w3.org/2000/svg" width="144" height="189" viewBox="0 0 144 189" fill="none">
        ...
        {
            svg: <svg xmlns="http://www.w3.org/2000/svg" width="213" height="114" viewBox="0 0 213 114" fill="none">
    ...
    return (
        <div className={`${style.orders} ${style.table}`}>
            <div className={style.kabina}>
                {stol.map((table, index) => (
                    <div key={index + 1}>
                        <div>
                            <div
                                onClick={() => isZanetTrue(index + 1) ? "" : handlColor(index + 1)}
                                className={`${color === (index + 1) ? style.green : ''} ${isZanetTrue(index + 1) ? style.color : ''}`}
                            >
                                {table.svg}
                                <span>{index + 1}</span>
                            </div>
                            <p style={{ color: isZanetTrue(index + 1) ? 'red' : (color === (index + 1) ? 'green' : '#333') }}>{isZanetTrue(index + 1) ? 'Забронировано' : ''}</p>
                        </div>
                    </div>
                ))}
            </div>
